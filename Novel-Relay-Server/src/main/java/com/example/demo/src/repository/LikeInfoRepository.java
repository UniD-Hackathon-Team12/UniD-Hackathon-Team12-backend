...
public interface LikeInfoRepository extends JpaRepository<LIKEINFO, Long> {
    ...
    @Query("update LIKEINFO l " +
            "set l.likeinfo_active = :likeinfo_active " +
            "where l.user.user_id = :user_id and l.novel.novel_id = :novel_id ")
    Integer changeActive(@Param("likeinfo_active") boolean likeinfo_active,
                         @Param("user_id")Long user_id, @Param("novel_id")Long novel_id);
    ...
    @Query("select l.likeinfo_active " +
            "from LIKEINFO l " +
            "where l.user.user_id = :user_id and l.novel.novel_id = :novel_id ")
    boolean getPresentLike(@Param("user_id")Long user_id, @Param("novel_id")Long novel_id);


    @Query("select l " +
            "from LIKEINFO l " +
            "where l.user.user_id = :user_id and l.novel.novel_id = :novel_id ")
    LIKEINFO getPresentLike2(@Param("user_id")Long user_id, @Param("novel_id")Long novel_id);
